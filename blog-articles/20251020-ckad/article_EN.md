...
The version is not hardcoded: it is read from a metadata.json file which, in a real scenario, might be generated by the CI pipeline — for example using GitVersion — which regenerates builds on releases, merges to develop, etc.
